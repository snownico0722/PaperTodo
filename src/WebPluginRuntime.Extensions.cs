...
internal sealed partial class WebPluginRuntime
{
    ...
    internal static bool AcceptsExtensionRequest(JsonElement root, JsonElement payload, string? expectedToken)
    {
        ...
        if (!method.StartsWith("paperActions.", StringComparison.Ordinal) &&
            !method.StartsWith("surfaces.", StringComparison.Ordinal)) return true;
    ...
    private object ExecuteSurfaceRequest(string method, JsonElement parameters) =>
        WebPluginSurfaceRequests.Execute((IPaperPluginSurfaces)_workspace, _workspace,
            _descriptor.Manifest!, method, parameters, Send);
