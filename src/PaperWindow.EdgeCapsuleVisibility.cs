...
public sealed partial class PaperWindow
{
    ...
    private bool ShouldCollapseExpandedDeepCapsuleFromEdgeClick()
    {
        return _controller.State.CollapseExpandedDeepCapsuleOnClick &&
            (HoldsDeepCapsuleSlotWhileExpanded || HasExpandedDeepCapsuleSlotReservation) &&
            IsExpandedPaperMeaningfullyVisible();
    }
    ...
    private bool IsExpandedPaperMeaningfullyVisible()
    {
        ...
        {
            var y = bounds.Top +
                (row + 0.5) * bounds.Height / EdgeToggleVisibilityGridSize;
            for (var column = 0; column < EdgeToggleVisibilityGridSize; column++)
            {
                var x = bounds.Left +
                    (column + 0.5) * bounds.Width / EdgeToggleVisibilityGridSize;
