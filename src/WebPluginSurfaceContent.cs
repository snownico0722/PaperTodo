...
/// the reviewed environment/local-origin and Workspace implementations. Navigation or renderer
/// failure cannot retain another document's bridge lease.
/// </summary>
internal sealed class WebPluginSurfaceContent : IPaperPluginSurfaceContent
{
    private readonly PaperBodyPluginManifest _manifest;
    private readonly string _entryPath;
    private readonly IPaperTodoHostApi _workspace;
    private readonly PaperPluginSurfaceContext _context;
    ...
    internal static string BridgeScript(string origin, bool closeOnEscape = false) => $$"""
        (() => {
          ...
          const close = () => { if (token) chrome.webview.postMessage({token, method:'surface.close'}); };
          window.papertodo = Object.freeze({
            surface: Object.freeze({close, post: message => request('surface.post', {message})}),
    ...
    private void OnNavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        _navigating = false;
        if (!e.IsSuccess) { Fail($"Web surface navigation failed ({e.WebErrorStatus})."); return; }
    ...
    private void OnMessage(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        try
        {
            ...
            if (method == "surface.close") { _context.Close(); return; }
            ...
            object? result;
            if (method == "surface.post")
            {
                var message = parameters.GetProperty("message").Clone();
                WebPluginSurfaceRequests.ValidateMessage(message);
                _post(message);
                result = new { delivered = true };
            }
            else result = WebPluginWorkspaceRequests.Execute(_workspace, method, parameters);
    ...
    private void Send(object message)
    {
        ...
        catch (Exception ex) { Trace.TraceWarning("Web surface message failed: {0}", ex.GetBaseException()); }
    }
    ...
    {
        e.Handled = true;
        if (!WebPluginRuntimeInfrastructure.IsSameOrigin(e.Uri, _origin)) OpenExternal(e.Uri);
    }
    private static void OpenExternal(string value)
    {
        if (!Uri.TryCreate(value, UriKind.Absolute, out var uri) || uri.Scheme is not ("http" or "https" or "mailto")) return;
    ...
    private void OnProcessFailed(object? sender, CoreWebView2ProcessFailedEventArgs e) =>
        Fail($"Web surface process failed ({e.ProcessFailedKind}).");
