...
- 所有贴边输入先作为 `EdgeCapsuleEvent` 经过 `EdgeCapsuleReducer`；不得在 `PaperWindow` 另写布尔状态机或绕过合法转移直接拼状态。
...
- per-window 的显示器 settle、标题 measure 和 pointer reconcile 共用一个 dirty/reconcile 调度入口；不得为新条件增加独立 pending/scheduled 布尔对。跨胶囊 arrange 只由队列协调器单独合并。
