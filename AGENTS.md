...
- 贴边 slot host 与内部 Chrome / Shell 使用同一真实宽度；`ClipToBounds` 只作动画安全边界，不得用来隐藏完整胶囊或关闭按钮。
- 贴边胶囊内容(图标+标题)朝屏幕内侧，关闭区在贴墙侧、悬停时从 0 宽度展开；跨队列拖拽的自由胶囊布局与贴边布局互斥，任何贴边落位前必须先退出拖拽视觉态并重置 `_appliedSlotEdge`。
