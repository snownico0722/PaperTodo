...
internal static partial class Program
{
    ...
    private static void MarkdownWorkerChecks()
    {
        ...
        Require(Glyphs(mathResult.Drawing).Any(glyph =>
                glyph.GlyphRun.GlyphTypeface.FontUri.ToString().Contains("KaTeX_", StringComparison.OrdinalIgnoreCase)),
            "worker formula uses bundled KaTeX glyphs rather than falling back to literal TeX source");
        ...
        Console.WriteLine("PASS worker STA RaTeX/text execution, frozen pixels (4 DPI values), duplicate requests, priority, cancellation and shutdown");
    }
