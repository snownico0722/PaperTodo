...
> API 2.2 补充：可用 `context.WorkspacePresentation` / `papertodo.workspace.request('papers.show', { paperId })` 控制其他已有纸片，不需要额外 permission。下面的 2.1 自身纸片接口和作用范围保持不变；完整接口与语义见中英文插件开发手册“跨纸片显示控制”。
