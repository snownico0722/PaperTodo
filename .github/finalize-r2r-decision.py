...
heading = "## D-036 — 单文件正式发布保持 ReadyToRun 关闭"
if heading in text:
    raise SystemExit(0)

index_line = "| D-035 | 冷渲染与预热共用唯一 artifact renderer | Accepted | Edge structure / lifecycle |"
replacement = index_line + "\n| D-036 | 单文件正式发布保持 ReadyToRun 关闭 | Accepted | 启动性能 / 发布 |"
if text.count(index_line) != 1:
    raise SystemExit("D-035 index anchor missing or duplicated")
text = text.replace(index_line, replacement, 1)
