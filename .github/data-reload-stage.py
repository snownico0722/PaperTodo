...
replace('tests/PaperTodo.DataReloadChecks/LiveChecks.cs',
    '            WriteLive(watched);\n            await Until(() => controller.State.Papers.Single(p => p.Id == "note-a").Title == "watcher title" && !controller.IsDataReloading, "watcher reload");',
    '            var activeWatcher = Field<FileSystemWatcher?>(controller, "_dataWatcher");\n            Require(activeWatcher is { EnableRaisingEvents: true }, "watcher was not active");\n            WriteLive(watched);\n            await Task.Delay(750);\n            var watcherStatus = controller.GetDataReloadStatusCore();\n            Console.WriteLine($"WATCHER pending={watcherStatus.PendingExternalChange} reloading={watcherStatus.IsReloading} outcome={watcherStatus.LastResult?.Outcome ?? "<none>"} title={controller.State.Papers.Single(p => p.Id == "note-a").Title}");\n            await Until(() => controller.State.Papers.Single(p => p.Id == "note-a").Title == "watcher title" && !controller.IsDataReloading, "watcher reload");')
