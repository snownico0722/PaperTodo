...
| E-017 | 2026-09-14 | 基于 #260 与主线的实际 WPF 分层源、原生 shape 接入 | Stopped / not adopted; failed experiment archived locally | D-039 |
...
## E-017 — 实际 WPF 分层源与原生 shape 接入（2026-09-14）

### 范围与当前状态

本地 `exp/edge-proxy-shape-260` 在 `26142c1d1d055c35e2b1c8843cee6030203a31cc`（#260 与当时主线整合）上接入路线 3。保留已整合的 render demand、静态预接管、代理复用及真实端点交接，未带入 E-016 未采用的 HWND 候选。旧三路原型及历史数据保持封存。

本实验分支的 Debug/Release 默认接入 native shape，Debug 可用 `PAPERTODO_EDGE_PROXY_SHAPE_EXPERIMENT=0` 做同包对照。默认接入是本地候选的代码状态，**不表示已经获得实机通过或合并资格**。2026-09-15 用户决定停止本路线，源码与结论保存在本地 `archive/edge-route3-failed-20260915`，不采用、不推送、不创建 PR。当前 Architecture 保持既有路线，失败取舍见 D-039；以下过程记录保留各自时点的证据状态。

### 接入边界

Host 通过一个有界、cloaked、不接收输入的 WPF atlas HWND 提供实际 VisualBrush 内容源；真实输入 HWND 保持正常布局与透明度。源将紧凑内容、按最终尺寸排版的正文、关闭符号与无文字外壳分开。Native helper 复用 Presenter 的同一 transition/QPC 合同执行外壳、裁剪、圆角及透明度；文字不缩放，也不建立第二份正文控件或业务 model。

源代与 slot revision 属于资源生命周期。普通收回保留已排版 Visual；替换须经全部存活消费者确认旧预览不可见，否则旧源保留到租约退休。新源首绘使用队列共享的 WPF 提交入口；同一真实 HWND 集合若换了 atlas，旧租约仍须保留到 root 替换的合成完成边界。原有输入成功当场转交、重试丢弃及真实端点验证继续保留。

代价包括每个获准 Host 的额外隐藏 HWND、atlas surface、VisualBrush 引用及换代时暂时保留的旧源。WPF 正文初次布局、内容动态绘制与最终交接并未消失。阴影切片、半透明叠加、不同 DPI、反向展开以及真实输入的像素与行为一致性仍需实测。

### 已有证据与未完成项

`输出/edge-route3-minimal-20260914/` 保存所有新源码快照、输入清单、构建、脚本和像素材料。`packages/route3-v1` 冻结 679 个文件；优化 Debug 日志包发布成功，EdgeTitle/EdgePreview/Lifecycle 的 Debug/Release 六组构建成功。首个构建存在三条新增 nullable 警告，随后在正式树集中修正；单文件发布另有既有 IL3000。构建成功不等于这些检查已经运行。

收尾后的 `packages/route3-v2` 同样保留完整源码清单；Debug/Release 编译均为 0 警告、0 错误，见 `compile-results.json` 与两份编译日志。该快照尚未发布新的单文件包或启动任何检查程序；冻结后仅补记本段编译结果。

先前共享真实 Root alpha 的候选被单独封存在 `packages/shared-source-rejected-v1`。受控 ROI 捕获在普通交接看见短暂变亮，插入 80ms 通道延迟时看见短暂变暗；初版夹具没有使用产品的完整 batch 回调，因此不能把这些时长当成实际产品交接时长。使用真实 batch 回调的纠正夹具已保存在 `packages/shared-source-batch-proof`，尚未运行。新分层源保持真实 Root alpha，不沿用共享源交接办法；其效果仍须实际验证。

用户使用电脑期间曾暂停录制与所有会显示窗口/操作输入的检查。`pause-replays.txt` 当时同时拦住 `Run-Replay.ps1`、`Run-Handoff.ps1` 和 `Run-Checks.ps1`。检查在发现已有日用 PaperTodo 进程时于启动前停止，未运行 route3-v1 的检查进程或录制。以下为用户恢复测试后的后续证据；旧暂停记录保留，不再代表当前状态。

### 2026-09-15：准备阶段回归定位与修复

恢复后 v2 的真实检查暴露了夹具初始化、源布局通知及原生分层像素问题，所有失败均保留。后续 v3 优先执行原录制器 OFF/ON，发现共同 30 动作的代理准备 P50/P95 从关闭时的 1.9607/17.5389 ms 升至 265.8014/348.4680 ms，不能据编译成功宣称接入完成。

主要原因是队列 placement 刷新每张 Host 的主题，而新增源逻辑无条件 `Invalidate`，即使颜色不变也使每轮 10 个 atlas 全部重建。现在按实际采样外壳样式判断换代，等值主题保留源。预览布局未就绪而暂用真实 HWND 时，Host 还会保留最后租约释放后的唯一有效当前 atlas；重新获取仍核验布局、owner、容量、DPI 与 native 身份，隐藏/关闭/失效照常回收，退休代最后租约仍关闭，原有 DComp 退休屏障不变。这是有界源缓存，没有屏幕或录制 owner 特判。同时修正同尺寸正文替换因描述值相等而漏增 Revision/漏发 Updated 的问题。

v7 的 Debug 检查构建与 Release 产品构建通过，源/主题/缓存生命周期回归通过。最终同包 OFF/ON 36 个 action/owner 完全一致、正常退出、日志零容量/文本丢弃。与旧 v3 ON 比较仍只取四组完全一致的前 30 动作：准备 P50/P95 为 v3 ON 265.8014/348.4680、v5 ON 45.3492/89.2661、v7 ON 27.4086/99.6240、v7 OFF 1.8993/18.8556 ms。第二步没有证明尾部全面改善；最终完整 36 动作的 v7 ON P50/P95 为 31.9784/99.6240 ms。

首次启动准备单样本 v7 ON 424.6578 ms，尚未优化；当前仍有部分预览 `source-unavailable` 和提交退休等待。WPF 更新间隔不等于 native 自主动画出屏帧率，完整像素/输入/多屏 DPI 验收仍未完成，因此不改主线已接受的架构结论或把路线 3 标记为通过。当前产品 render demand 正常启用，脚本旧 `RenderDemand=false` 元数据不控制产品，该事实已由 `scheduler.demand.mode=1` 核对。

完整数据、开关、SHA、软件间隔各分位数、CPU/内存与局限见 `输出/edge-route3-minimal-20260914/performance-v7-r1/结果.md`；旧 v3 报告、v4/v5/v6 失败、v7 成功检查和所有冻结源均保留。没有提交、推送或新建 PR。

### 单胶囊独立动画能力验证

用户要求先判断核心机制是否有价值后，新增 opt-in `--proxy-animation-value`，复用实际 Host 与 DXGI ROI 夹具，只跑已准备好的真实 HWND 对照与 native shape 两组展开/反向，无完整鼠标录制。两组两段 UI 阻塞期间的 Host.Apply 和原生 shape 再提交增量均为 0；严格 QPC 且排除段首 75ms 的样本中，真实 HWND 的展开与反向均保持同一幅图像（8/28 个有效样本），native shape 的展开/收回仍有宽高及正文显露变化（11/29 个有效样本，11/23 幅不同 RGB 图像）。这是独立动画能力的正面证据，不是物理 FPS 或整体响应延迟收益。

原生静态起点仍有 119 个像素超过 RGB 容差 2；UI 阻塞的收回末端勾形图标未及时恢复。因此能力成立，像素正确性与实际产品接入仍未通过。准备成本和 owner fallback 没有被这个预先准备的实验解决；本次只涉及单边缘、单样例、每组一次。原始像素、QPC/SHA、冻结检查程序和原尺寸固定选样图完整保留于 `输出/edge-route3-minimal-20260914/animation-value-r1/`，结论见其中 `结论.md`。

### 最终处置：失败记录分支，停止采用（2026-09-15）

失败指这版产品接入未达到采用条件，不否认预先准备后可以独立动画的能力。修复消除了本实验无条件主题失效造成的大量重建，但没有证明比关闭路线 3 更好的整体体验；继续完善需要承担额外资源生命周期、布局就绪、交接和像素一致性成本。用户决定在这里停止，不再继续验收或推进合并。

| v7 同包 OFF / ON，完整同序 36 动作 | OFF | ON |
| --- | ---: | ---: |
| 代理准备 P50 / P95（ms） | 1.8993 / 18.8556 | 31.9784 / 99.6240 |
| 首次启动准备（ms，单样本） | 52.5088 | 424.6578 |
| 整轮进程 CPU 时间（s） | 6.703125 | 9.015625 |
| 终点 Private Bytes（MiB） | 269.828125 | 314.25 |

每组仅一轮，上表不承诺可推广的固定差值；跨 v3 的统计仍只使用共同 30 动作。尚未完成的实际输入、多屏/混合 DPI 和完整像素验收记录为未通过采用条件，不能改写为测试通过，也不能把未运行场景说成已观测失败。

- 失败分支以 `26142c1d1d055c35e2b1c8843cee6030203a31cc` 为父基线，保留完整产品候选、真实 WPF/DXGI 检查、诊断入口及本记录。#260 既有保留代理/静态预接管与主线 render demand 不因停止路线 3 而撤销。
- `输出/edge-route3-minimal-20260914/packages/route3-failed-archive-20260915/` 冻结最终源、父提交、diff 与逐文件 SHA；原有源码快照、构建、成功/失败日志、ROI BGRA、QPC、分析器和原始测试数据保持原处，未放入 Git。
- v7 构建及生命周期检查、单胶囊能力捕获的验证范围见上文。能力捕获后源码只有对应的 nullable 空值断言标记修正；归档没有重新运行录制或扩大验收，不把冻结快照写成新测过的版本。
- 工作目录回到引入路线 3 前的上述基线；停止状态与本地归档入口见输出根目录 `FAILED-RECORD.md`。没有产品发布，`CHANGELOG.md` 不增加用户功能，未采用的架构草案不升格为 Architecture。
