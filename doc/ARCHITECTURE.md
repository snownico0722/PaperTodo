...
`SkinBorder.CaptureHost` 从真实 `HwndSource` 取得当前分层辅助表面的 HWND（不使用菜单 owner 代替 popup），观察宿主移动、DPI、可见性及祖先 opacity；隐藏／关闭／部分透明时停止处理，恢复后继续，卸载解除全部订阅。`SkinBorder.Refraction` 现在只承担云母／亚克力／透色亚克力／描图纸辅助窗口的有限范围背景柔化，不再包含专属折射 shader、色散、位移表或光学参数。`DesktopLensCapture` 仅在本机内存中用 SRCCOPY 采样 DWM 合成后的局部 SDR 背景，不使用可能扰动鼠标显示的 legacy CAPTUREBLT；后台保持单最新帧邮箱与像素预算，UI 用零等待 TryLock，移动时只更新场景映射或唤醒后台，不提高固定采样频率。采样期间持有 WDA_EXCLUDEFROMCAPTURE 排除自身，结束后恢复原 affinity；独立“实时背景处理”开关关闭时所有这些辅助表面停止采样。Aero、展开纸片和设置窗口不走软件采样。每个 popup 单独拥有资源，关闭子菜单不影响 parent。`MaterialMenuOpening` 只延后需要软件背景的 ContextMenu／Popup 首次打开请求，在 HWND 出现前异步取得局部背景；失败或超时本次使用静态回退，下次重新尝试，不创建预热 HWND、不改前景 opacity，也不改变 WPF 定位／焦点／菜单关闭语义。设置页只替换外壳内的内容树，同一 SkinBorder 与 native adapter 保留，普通切页不强制重装 DWM。真实 Windows 11、HDR、混合 DPI 和高刷新率观感仍需人工验收。
