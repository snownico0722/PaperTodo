...
未命中预热时，普通短行使用精简的 WPF 文字元素；长行、长代码行与样式密集段落通过惰性共享的 `MarkdownLayoutWorker` 在独立 STA 上调用 `TextFormatter`，只准备可见行。正式需求优先于预热，等价的在途段落可共享结果，后台按行让出并检查取消；没有任务时只等待 Dispatcher。

`MarkdownEdgePreviewPreload` 按有界摘要的字符量和样式内容筛选来源，在共享的一次性合并延迟后逐个准备整张预览的不可变排版绘制结果。UI 捕获已物化片段、字体、DPI 和资源冻结副本；同一个 STA worker 完成文字排版，UI 最后组合冻结 Drawing 与链接矩形。预热不创建隐藏 WPF 卡片、正文控件树或第二个窗口，也不让 worker 持有 UI 控件、可变缓存或业务回调。

每个合格来源最多保留一份当前 artifact，不按鼠标邻居预测或固定张数淘汰。版本、摘要、字体、缩放、DPI、资源及正文宽度必须匹配；高度在当前卡片上限内由实际 viewport 裁剪，因此较矮卡片可复用同一宽度的结果。正式显示创建一个绘制面及链接的原生交互元素，完全裁掉的链接禁止输入，artifact 留在缓存中，不转移或归还 WPF 正文 ownership。链接与冷路径共用 `MarkdownPreviewLinkHit`，由 WPF 持有按下、释放、焦点和键盘行为；背景点击仍交给既有纸片手势。

`MarkdownEdgeCapsulePreviewViewport` 唯一拥有这一视图的准备、取消与完整发布。命中时挂载绘制面，未命中时异步等待现有分批渲染；两者都不在等待期间持有动画队列屏障。完整发布前不暴露正文交互，收起立即关闭交互并取消未完成工作。同一视图、内容和尺寸的已完成结果可在短暂收起/恢复间复用；内容、外观、DPI 或尺寸变化后重新准备，卸载释放挂载的元素，迟到任务不能覆盖新版。

来源关闭、移出队列或缓存清空后撤销预热资格；正常内容/外观或宿主生命周期事件可以再次提交。正式浏览会打断预热，仍有效的排队请求保留；资源变化导致的旧 artifact 直接丢弃，不建立自动重试或轮询协议。窗口 `Describe`、行内语义、普通冷路径短行和最终 UI 挂载仍在 UI；该缓存不接管队列、动画、窗口几何或持久化，也不改变 D-027 的编辑正文同步语义发布。背景与装饰沿用现有受限预览语法，冷路径保留为实际未命中路径及画面对照，不创建另一套 Markdown parser。取舍见 D-033 / D-034。
