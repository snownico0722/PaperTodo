...
液态玻璃的真实背景折射由 `SkinBorder.Refraction` 持有，偏好 `AppState.LiquidGlassRefraction` 经现有 `StateStore` 保存。仅可见、展开、不透明、已启用原生合成的普通 `PaperWindow` 使用；设置和各类胶囊不启动采样。位于 `Border.Child` 前的容器按「采样背景 → 表面染色／反光／owner 描边 → 正文」排序，避免不透明背景视觉盖住父 Border 的材质反光；表面绘制缓存按材质／几何刷新，不新增定时器。统一背景避免透明中心与不同步的边缘拼接。`LiquidRefractionEffect` 用少量双线性采样进行轻散射和饱和度处理，只有背景有极轻的整体放大，正文与按钮始终不参与 shader；边缘使用 `LensDisplacement` 共享 512×1 的五次平滑曲线，端点一阶和二阶导数归零，浅曲面温和向内折射，内角法线平滑过渡。`GlassMetrics` 按 DIP、短边和面积连续计算厚度、位移、散射、遮色、饱和度和背景放大幅度；大阅读面板增加散射与遮色，窄长面板保持轻薄，不拉伸圆角或生成全尺寸位移图。正文／编辑器／命中／布局／HWND 归属不变。
