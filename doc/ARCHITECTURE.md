...
普通短行使用精简的 WPF 文字元素。长行、长代码行与短但样式密集的行通过共享、惰性启动的 `MarkdownLayoutWorker` 在独立 STA 上调用 `TextFormatter`，只准备可见行。UI 提供已物化行内片段、字号/字体/DPI/资源冻结副本与正文区域；后台返回冻结 Drawing、实际尺寸、截断状态及链接矩形，不能持有控件、可变缓存或 UI 回调。正式需求优先于预热，相同的在途请求可共享结果，按行让出后台队列并检查取消；无任务时只等待 Dispatcher。

`MarkdownEdgeCapsulePreviewViewport` 异步等待重段落后校验当前版本并一次发布，不在等待期间持有动画队列屏障。现有 UI 正文预热缓存与独占移交不变；窗口尺寸 `Describe`、行内解析、普通短行与最终挂载仍在 UI，后台布局不接管窗口几何，也不改变 D-027 的编辑正文同步语义发布。显式同步 renderer/checks 使用同一布局内核，不是 live preview 等待后台的退路。没有完整 AvalonEdit 预览控件、另一套行内解析器或第二套发布 owner。
