...
- **Experimental skins**: Settings → Appearance → Skin separates surface material from color scheme. Keep default paper, native Mica and the two Acrylic options, or try pearl/holographic foil, tracing paper, liquid-glass approximation, ceramic glaze, Aero glass and pixel styling. Standard Mica and both Acrylic skins keep their original neutral palette; decorative skins restore the independently saved color choice. Legacy Mica settings retain their appearance. Pearl sheen drifts slowly while stationary and reacts to movement; motion stops when animations are disabled or surfaces are hidden/minimized; pixel borders, shadows and buttons keep the normal body font. Liquid glass now uses clear composition instead of frosted blur, but does not perform real refraction. Paper and capsule shells share painting without changing docking or transition ownership. Material continues beneath the header without a separate gray plate or white seam. Aero has soft broad diagonal reflections, ceramic has soft glazed light and shallow shading, liquid glass has pointer-directed hairline reflections, and pixel styling uses a single frame and physical-pixel icons. Thick inset frames are removed. Host-owned focus/open-edge borders remain intact.
