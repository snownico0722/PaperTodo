...
- **Live backgrounds**: Papers, settings, capsules and menus share glass effects. Capsules and menus offer full material or a quieter, more opaque paper-like base for readability. Edge previews use a fixed hue and opacity without live capture; color families are tuned for each material. Master, paper and submenu templates prepare the current material before opening. Settings pages retain their background, and skin settings use concise controls and help icons. A wider scene is sampled at low rate while movement and highlights reproject on display frames, retaining a fixed screen grid. Samples stay in local memory and are never saved or uploaded. Capture can exclude these surfaces from screenshots, recordings and screen sharing; turn it off before sharing. Mica/Acrylic auxiliaries approximate background diffusion; Aero does not capture the desktop.
