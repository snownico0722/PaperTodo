...
- **Experimental skins**: Eight choices remain: paper, Mica, both Acrylic variants, tracing paper, liquid glass, Aero and pixel styling, all with independent colors. The retired glazed skin and its rendering/resources are removed; its saved ID falls back to paper. Liquid glass has smooth curved-edge refraction, size-aware directional highlights and true background RGB dispersion. Its centre stays at the original background scale when the surface is resized. Small capsules avoid over-compressing the edge; pointer lights stay circular regardless of aspect ratio and return to ambient lighting on leave. The reflective finish stays above the captured scene. Aero retains clear tint and soft reflections without capture or frosting.
- **Live backgrounds**: Papers, settings, capsules and menus share glass effects. Capsules and menus offer full material or a quieter, more opaque paper-like base for readability. Edge previews use a fixed hue and opacity without live capture; color families are tuned for each material. Master, paper and submenu templates prepare the current material and initial texture before opening, without the system popup fade. Settings pages retain their background, and skin settings use concise controls and help icons. A wider scene is sampled at low rate; small movements reuse it while motion and highlights follow display frames, reducing redundant transfers and drawing. Scene relocation publishes pixels and coordinates together; capture avoids the legacy cursor-disrupting flag. Samples stay in local memory and are never saved or uploaded. Capture can exclude these surfaces from screenshots, recordings and screen sharing; turn it off before sharing. Mica/Acrylic auxiliaries approximate background diffusion; Aero does not capture the desktop.
