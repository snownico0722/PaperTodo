...
- **Experimental skins**: Eight choices remain: paper, Mica, both Acrylic variants, tracing paper, liquid glass, Aero and pixel styling, all with independent colors. The retired glazed skin and its rendering/resources are removed; its saved ID falls back to paper. Liquid glass has smooth curved-edge refraction, size-aware directional highlights and true background RGB dispersion. Small capsules avoid over-compressing the edge; pointer lights stay circular regardless of aspect ratio and return to ambient lighting on leave. The reflective finish stays above the captured scene. Aero retains clear tint and soft reflections without capture or frosting.
- **Live background processing**: Papers, Settings, capsules, context menus and submenus share the liquid-glass pipeline. Disabling full auxiliary strength still processes the real background, with weaker refraction, dispersion, diffusion, tint and highlights. Layered Mica/Acrylic auxiliaries approximate the material with local background diffusion; main papers retain DWM. Aero auxiliaries use alpha transmission. Foreground, main-paper strength, HWNDs and hit areas remain unchanged. Ordinary panels prefer physical-pixel sampling; larger panels use a fixed screen-space downsampling grid with a bounded budget, single-frame mailbox and nonblocking upload. Menus prepare their first real background before opening, and Settings keeps the material shell alive across page changes. Sampled surfaces are excluded from some screenshot/recording/sharing APIs; disable Live background processing when sharing. Frames stay in local memory and are never saved or uploaded. Hidden, minimized, partially transparent or failed surfaces stop processing; closing a popup does not stop its owner.
