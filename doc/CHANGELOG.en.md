...
- **Experimental skins**: Appearance → Skin offers default paper, Mica, both Acrylic variants, tracing paper, liquid glass, ceramic, Aero and pixel styling. All skins support independent color schemes; Neutral keeps the original system colors for Mica and Acrylic. Capsules, context menus and submenus follow the selected material with softer effects by default; a new switch restores full material strength without fading text or icons or changing hit areas. Aero uses clear blue-tinted transmission and soft fixed-width diagonal reflections with movement parallax, not heavy Acrylic blur, desktop capture or screenshot exclusion. Ceramic separates an opaque ivory body from localized clear-coat reflections. Pixel styling retains normal fonts. No thick inset frame.
- **Live background refraction**: Liquid glass has a shallow shoulder that joins the center smoothly, subtle background magnification and scattering, and a clear-coat reflection above the sampled background. Text and buttons are never transformed. Optical depth and tint adapt continuously to panel size, with narrow papers remaining light. One coherent background texture is bounded to 196,608 pixels. Unchanged pixels do not upload or keep a WPF frame callback attached, and bitmap access does not block the UI. An independent setting disables capture. Frames remain in local memory without saving or uploading. Active liquid papers are excluded from some screenshot, recording and screen-sharing APIs to prevent feedback; disable the option when sharing. Hiding, minimizing, collapsing, partial opacity, leaving the skin or capture failure releases sampling and restores a static fallback. Capsules, menus and Settings do not capture. Not an exact reproduction of Apple Liquid Glass; clarity and text contrast still depend on the backdrop.
