...
- **Live backgrounds**: Papers, settings, capsules and menus share glass effects. Capsules and menus offer full material or a quieter, more opaque paper-like base for readability. Edge previews use lightweight materials without live capture. Menus start with their material ready, settings pages retain their background, and skin settings use concise controls and help icons. Ordinary panels prefer native-pixel samples; large panels use a fixed screen grid. Samples stay in local memory and are never saved or uploaded. Capture can exclude these surfaces from screenshots, recordings and screen sharing; turn it off before sharing. Mica/Acrylic auxiliaries approximate background diffusion; Aero does not capture the desktop.
