...
- **实时背景处理**：纸片、设置、胶囊和菜单共享玻璃效果。胶囊与菜单可选完整材质；关闭时效果更轻、纸片底色更实，更便于阅读。边缘浏览使用固定色调与透明度的轻量材质；配色按不同材质分别调校。主胶囊、纸片和子菜单打开前准备当前材质，设置切页保留已有背景；皮肤设置精简为选项和提示图标。背景低频采样较大范围，小幅移动复用已有场景，拖动与光效按显示帧呈现，减少重复传输与绘制。采样仅在本机内存处理，不保存或上传；可能影响截图、录屏和共享，分享前请关闭。云母／亚克力辅助表面为局部背景柔化近似；Aero 不采样。
