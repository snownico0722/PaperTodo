...
- **输入节奏优化**：优化了空行限制规则，允许连续创建并保留多个空待办行。
...
- **右键菜单优化**：重写了删除确认弹窗，采用更窄的纸张风格细滚动条，统一了窗口边框、阴影、圆角和悬停反馈，并消除了 Windows 默认控件的蓝色预选框与焦点框。
...
- **自启动管理**：设置中增加了开机自启动选项，支持安全地读写当前 Windows 用户的注册表启动项。
...
- **外部打开按钮同步后缀**：设置中修改笔记外部打开后缀后，标题栏按钮会同步显示对应的文件后缀；后缀超过两个字母时只显示前两个字母。
...
- 修复笔记纸标题编辑后，再次点击标题输入框仍可能无法正常定位光标的问题。
