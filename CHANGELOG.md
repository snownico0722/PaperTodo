...
- **原生云母皮肤**：在「设置 → 视觉 → 配色方案」选择云母，退出并重新打开后，展开的纸片与设置窗口使用 Windows 原生材质，可选标准云母、标准亚克力和更通透的透色亚克力，支持浅色、深色与跟随系统；可勾选“材质始终显示激活效果”，失焦后保持材质外观。不读取或模拟壁纸。需要 Windows 11 22H2 及以上；不支持的系统、关闭透明效果或高对比度时使用实色。胶囊、折叠动画和半透明状态保留实色绘制，不改变贴边胶囊的动画结构。
