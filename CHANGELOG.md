...
- **皮肤实验**：在「设置 → 外观 → 皮肤」选择默认纸片、标准云母、标准亚克力、透色亚克力、半透明描图纸、仿液态玻璃、陶瓷／奶油釉面、Aero 玻璃和像素风。五种装饰皮肤可独立配色，标准原生材质保留素灰配色，切换不会覆盖已保存的配色。描图纸带乳白纤维纹理，Aero 使用低染色系统模糊、淡蓝透色和有明确亮暗边界的斜向反光，反光宽度固定并随移动产生视差；陶瓷区分不透明奶油瓷底与局部透明釉反光，曲面边缘具有连续的明暗变化，像素风使用紧凑阶梯角、方形控件和物理像素图标。正文保留普通字体，外壳不叠加宽厚内套框。
- **实时背景折射**：仿液态玻璃使用统一背景的轻微散射、透色和温和边缘折射，正文保持清晰；厚度、模糊与遮色随纸片尺寸连续变化，窄长纸片不会被当作大面板处理。单次背景采样限制在 196,608 像素，替代四条独立边缘；未变化背景不重复上传，主线程不等待位图锁，空闲时解除呈现回调。设置有独立开关。背景仅在本机内存中处理，不保存或上传；为避免自身重影，开启时该纸片会被部分截图、录屏和屏幕共享接口忽略，需要分享时应关闭。隐藏、最小化、收起、部分透明或切换皮肤会停止；不支持或采样失败时回退静态材质。胶囊和设置窗口不启用实时折射，不等同于苹果 Liquid Glass。Aero 不启用采样，保留正常截图行为；系统模糊失败时回退不透明表面。
