...
- 数据文件缺少核心纸片列表或该列表为 null 时不再被当作首次启动的空数据；程序会尝试读取备份，主文件和备份均无法读取时停止启动，避免默认纸片覆盖原文件。
...
- 基础行为页恢复默认后，已经打开的纸片会立即同步提示、关联笔记入口、标题限制、置顶策略和边缘胶囊布局，不再需要额外操作或重启才能完全生效。
- 修复边缘胶囊在跨显示器拖动、混合 DPI 交接或拔掉副屏后可能出现的激发态描边缺失、反向残留、未贴边和被系统移到屏幕中间的问题；跨队列拖拽松手后会从当前位置平滑吸附到目标槽位，最终贴墙交接不再瞬间闪现或短暂消失；整个交接的目标重定向和失败恢复均有统一次数上限，系统暂时无法确认目标窗口位置时会安全结束交接，避免提前撤掉悬浮遮盖，也不会无限残留悬浮胶囊。
...
- 程序异常退出时只在恢复文件确实写入成功后提示“已保存”；恢复文件创建失败时会明确提示检查目录写入权限和剩余空间。
