...
- 修复剪贴板中只有图片（如截图）时无法粘贴进笔记的问题，现在可直接按 Ctrl+V 粘贴图片。
- 修复编辑态下对图片点右键会被误判为失焦、自动切回浏览态的问题，图片菜单关闭后会停留在编辑态。
- 边缘胶囊的右键菜单会稳定显示在其他置顶窗口之上，不再偶发被窗口遮挡。
- 优化 Windows 分屏贴靠显示：贴靠时自动隐藏纸片阴影和边距；分屏或最大化状态下折叠时会解除该状态，折叠后拖动胶囊不再被误放大。
- 分屏纸片折叠为胶囊后即视为普通纸片，再次展开会恢复到分屏前记录的纸片大小，而不是重新占满分屏格子。
