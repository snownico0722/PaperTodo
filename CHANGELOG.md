...
- **皮肤实验**：在「设置 → 外观 → 皮肤」选择默认纸片、标准云母、标准亚克力、透色亚克力、半透明描图纸、仿液态玻璃、陶瓷／奶油釉面、Aero 玻璃和像素风。所有皮肤均可独立配色，云母和亚克力的素灰选项保留系统原色。胶囊、右键菜单及子菜单跟随材质，默认弱化效果，可开启「胶囊与右键使用完整材质强度」；文字、图标和点击区域不变。描图纸带纤维纹理；Aero 使用清透蓝色玻璃、柔和的固定宽度斜反光与移动视差，不再沿用亚克力重磨砂，也不增加桌面采样或截图排除；陶瓷区分奶油瓷底与局部釉面反光；像素风使用阶梯角和物理像素图标。正文保留普通字体，不叠加宽厚内套框。
- **实时背景折射**：仿液态玻璃以浅曲面平滑过渡到中心，统一背景源提供轻微放大、柔化和边缘折射，表面反光位于采样背景之上，正文与按钮不参与变形。厚度、散射与遮色随纸片尺寸连续变化，窄长纸片保持轻薄。单次背景纹理上限为 196,608 像素，未变化背景不重复上传，主线程不等待位图锁，空闲时解除呈现回调。设置有独立开关。背景仅在本机内存中处理，不保存或上传；为避免自身重影，开启时该纸片会被部分截图、录屏和屏幕共享接口忽略，需要分享时应关闭。隐藏、最小化、收起、部分透明或离开液态皮肤会停止；不支持或采样失败时保留静态材质。胶囊、菜单和设置窗口不启用实时折射，不等同于苹果 Liquid Glass。
