...
- **皮肤实验**：保留默认纸片、云母、两种亚克力、描图纸、液态玻璃、Aero 和像素风八种皮肤，均支持独立配色；移除旧釉面皮肤及其绘制与资源，旧选择回退默认纸片。液态玻璃具有平滑曲面边缘、随尺寸适配的方向高光和真实背景 RGB 色散；小胶囊不再过度挤压边缘背景，光斑不随长宽比拉伸，指针离开后恢复环境高光，表面反光始终位于采样背景之上。Aero 保留清透染色和柔和斜反光，不增加磨砂或桌面采样。
- **实时背景处理**：纸片、设置、胶囊、右键菜单和子菜单共用液态玻璃处理；「胶囊与右键使用完整材质强度」关闭时仍处理真实背景，仅减弱折射、色散、柔化、染色与高光。云母／亚克力的分层小窗口用局部背景柔化近似，主纸片仍使用 DWM，Aero 小窗口使用透明染色。文字、图标、主纸片强度、窗口和点击区域不变。普通面板优先原像素采样，大面板使用固定屏幕网格降采样，并保留有限预算、单帧邮箱与非阻塞上传。菜单在后台准备首帧后直接显示材质，设置切页保留已有材质外壳。开启采样的表面会被部分截图、录屏和共享接口忽略；共享时关闭「实时背景处理」。背景仅在本机内存处理，不保存或上传；隐藏、最小化、部分透明或失败时停止，关闭菜单不会停止其所有者的材质。
