...
- 设置窗口拆分为「基础行为 / 视觉 / 快捷键」三页，高度按三页真实内容测量后取最高一页，切换分页时不再忽高忽低；常规显示器上完整铺开不出现滚动条，仅当超出工作区时才滚动。右下角署名改为贴边显示，不再占用整行；调整整体字号时保持窗口顶部不动，只向下扩展。Markdown 显示仍在基础行为页，分区顺序按日常使用与胶囊优先整理。视觉页可按 5% 调整 80%–120% 的整体字体大小，并分别设置笔记正文、待办正文、标题和胶囊文字的小中大字号与加粗；各组共用一个说明图标并有轻分隔。单个笔记原有的百分比缩放会在全局与正文字号基础上继续叠加，待办行的内边距、最小高度、勾选列和勾选框也会随整体字体大小同步缩放。
...
- 每项全局快捷键均可独立启用；录入、清空、恢复默认、开关切换都会立即生效并自动保存，不再需要单独点保存。录入组合键时会自动勾选启用；单项或整表恢复默认时按默认启用状态写入（边缘序列默认关闭）。新增默认关闭的「快速启动侧边胶囊」：左侧队列默认 `Ctrl+Shift`、右侧队列默认 `Ctrl+Alt`，每侧一行设置；可自定义 2～3 个修饰键，数字 1–9 固定显示且不可改，左右不能使用相同修饰键，开关一次启用本侧整组。调用时优先查找鼠标所在显示器的指定侧与对侧，再按左邻、右邻顺序搜索其他显示器，主胶囊不计入序号；拖动任意边缘胶囊期间会忽略这些快捷键。
