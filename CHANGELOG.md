...
- **贴边胶囊多屏 / 左右侧（多队列）**：每条屏幕边各有一条独立的贴边胶囊队列，每条队列有自己的「收起全部」主胶囊。直接拖动**单个**贴边胶囊即可把它移到屏幕左侧 / 右侧或其他显示器——松手时自动吸附到光标所在显示器的最近边并加入那条队列，不会停在屏幕中间。队列归属会被记住，显示器拔除后自动回退到最近的屏幕。「收起全部」主胶囊只在自己这条队列内上下滑动调整起始高度。

**胶囊相关问题修正**
- **贴边胶囊起始高度与位置**：拖动「收起全部」主胶囊可在所属队列内上下滑动调整起始高度；拖动单个贴边胶囊可改变其所在屏幕边与显示器。
