...
- 边缘胶囊与 slot 0 主胶囊改为按实际显示宽度绘制单侧圆角贴边标签，并在目标显示器坐标中一次提交完整物理像素矩形；关闭区固定在屏幕墙边并随窗口实际宽度同步伸缩，拖出队列时改用独立悬浮拖拽窗口，贴边胶囊不再通过裁切、屏幕外溢出或复用悬浮布局形成外观，修复竖屏、混合缩放、右侧悬停闪烁及拖拽归位后的裁切和形态残留。
...
- 边缘胶囊关闭区恢复到屏幕墙边；右侧胶囊伸缩时不再因原生窗口移动产生反复悬停切换和位置闪烁。
