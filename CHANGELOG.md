...
- 修复边缘胶囊在跨显示器拖动、混合 DPI 交接或拔掉副屏后可能出现的激发态描边缺失、反向残留、未贴边和被系统移到屏幕中间的问题；跨队列拖拽松手后会从当前位置平滑吸附到目标槽位，最终贴墙交接不再瞬间闪现或短暂消失；系统暂时无法确认目标窗口位置时会经过一次有上限的最终确认再结束交接，避免提前撤掉悬浮遮盖，也不会无限残留悬浮胶囊。
