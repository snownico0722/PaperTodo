...
- **边缘胶囊实时预览**：鼠标悬停在边缘胶囊上即可展开轻量、可交互的预览卡片，无需打开完整纸片即可即时浏览内容。
  - **待办预览**：实时显示当前筛选下按原顺序排列的前 12 项，支持滚动浏览、勾选完成或撤销；可在当前卡片临时切换「仅未完成」，不改变正文或待办顺序。底部显示未展示的事项数量，点击「打开完整纸片」或卡片背景即可查看全部内容。
...
- **待办连续多选与批量操作**：在待办正文或行内空白处拖动，跨条目后即可连续选择多条事项，支持批量勾选完成/恢复、批量复制、右键删除全部或拖拽整组丢弃。选中整项待办后，`Ctrl+C` 保持纯文本复制，`Ctrl+Shift+C` 可复制为保留完成状态、顺序和多行文字的 Markdown 任务清单。
...
- **关联纸片显隐恢复**：高级模式可保留关联纸片在快捷隐藏前的隐藏状态；同一次隐藏周期内重复执行「隐藏全部」仍保留首次记录，恢复时不会意外弹出原本隐藏的关联窗口。
