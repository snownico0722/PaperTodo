...
- **实时背景处理**：纸片、设置、胶囊、右键菜单和子菜单共用液态玻璃处理；「胶囊与右键使用完整材质强度」关闭时仍处理真实背景，仅减弱折射、色散、柔化、染色与高光。云母／亚克力的分层小窗口用局部背景柔化近似，主纸片仍使用 DWM，Aero 小窗口使用透明染色。文字、图标、主纸片强度、窗口和点击区域不变。背景纹理保持 196,608 像素预算、单帧邮箱与非阻塞上传。开启采样的表面会被部分截图、录屏和共享接口忽略；共享时关闭「实时背景处理」。背景仅在本机内存处理，不保存或上传；隐藏、最小化、部分透明或失败时停止，关闭菜单不会停止其所有者的材质。
