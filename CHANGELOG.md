...
- **Experimental skins**: Seven choices remain: paper, Mica, both Acrylic variants, tracing paper, Aero and pixel styling, all with independent colors. Retired skin IDs fall back to paper. Aero retains clear tint, bounded reflections and position-aware parallax without desktop capture or frosting.
- **Auxiliary background processing**: Layered capsules and menus for Mica, Acrylic, Clear Acrylic and tracing paper can process a bounded local background for material diffusion. Full and quieter material strengths share the same foreground and hit testing. Samples stay in local memory and are never saved or uploaded; sampled windows can be excluded from screenshots, recordings and screen sharing, so disable this before sharing. Expanded papers and Settings use native backdrops instead of software capture. Aero does not capture the desktop.
