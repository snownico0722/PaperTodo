...
- 修复通过全局快捷键、托盘“显示全部”或显隐切换重新显示纸片时，开启“记住边缘胶囊展开位置”的纸片可能无法回到上次展开位置的问题；同时优化多纸片批量恢复，减少重复胶囊重排，并同步刷新待办关联笔记的显示状态。
- Markdown 裸 `http://` / `https://` 地址可直接点击打开；行内代码和围栏代码中的示例地址保持普通文本，句末标点、括号和成对强调标记不会混入实际网址。
- 待办新增“已完成自动置底”：完成后移到已完成区域末尾，取消完成时回到未完成区域末尾；开启自动清除时暂不执行。
- 待办原“关联笔记”扩展为关联任意内置纸片（待办纸或笔记纸）；继续使用旧 `linkedNoteId` 字段兼容已有 3.x 笔记关联数据。新建的待办纸关联若降级到旧版运行，可能被旧版加载规则清除。
- 待办项可拖入一个文件或文件夹作为快启路径；可直接打开、打开所在位置或解除关联，失效路径会明显提示；快启路径与纸片关联互斥。
- 仅带纸片关联或快启路径的空文字待办会继续保留，不再被空项清理或 Backspace 静默删除。
- 设置新增界面语言选择，可选择跟随系统、简体中文、English、日本语或韩语；保存后重启生效，并修复异步界面刷新时语言可能退回系统语言的问题。
...
- **含图笔记渲染性能优化**：修复含图片笔记在前台静止时，视口细微变化可能触发连续无意义重绘、导致 GPU 占用异常偏高的问题。
- **大图导入内存优化**：导入和压缩大图时不再提前完整解压像素，需要压缩时直接按目标尺寸解码，明显降低瞬时内存峰值。
- 修复调整 Windows 显示缩放、DPI 或跨显示器后，笔记图片可能没有及时重新排版并按新 DPI 重新解码的问题。
- 减少笔记预览 / Markdown 模式的重复刷新和待办右键菜单的重复创建，降低无意义界面开销。
- 恢复待办右键菜单的键盘入口，可使用菜单键或 `Shift+F10` 打开。
- 提升旧版 Windows 启动兼容性，避免部分环境在程序界面出现前直接启动失败。
- 修复贴边深胶囊右键菜单打开二级菜单后，点击子项可能被外部点击保护提前关闭的问题。
