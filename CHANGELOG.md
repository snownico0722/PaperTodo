...
- **边缘胶囊实时预览**：鼠标悬停在边缘胶囊上即可展开轻量、可交互的预览卡片，无需打开完整纸片即可即时浏览内容。
  ...
  - **笔记预览**：跟随 Markdown 渲染设置，关闭时显示原文，基础提供轻量高亮，增强淡化语法并显示列表圆点、分割线，编号和任务状态保持清晰，完全渲染隐藏语法标记；支持标题、强调、删除线、代码块、列表与图片占位。文字沿用正文的字体、字号、加粗、文字渲染设置及单篇缩放，普通行和空行不额外加宽间距。卡片只展示当前空间内的内容，不支持滚动，溢出时在底部显示省略号；长笔记最多准备前 16 个内容块、6000 字符，卡片尺寸与显示使用同一份预览内容，不为预算外内容预留空白；按可见空间分批准备文字，纯文本减少不必要的临时分配，密集样式短行也避免一次性构造大量文字对象；相同内容和尺寸的预览在短暂收起后恢复时复用已完成结果，内容、字体、缩放或屏幕变化后重新准备。行内语法沿用正文的识别实现，改善嵌套强调、转义与复杂链接的一致性。启动恢复和内容变化后，会在空闲期提前准备较长或样式较多的边缘笔记预览，减少首次浏览等待；轻内容不主动预热，离开边缘队列后释放对应缓存。预热与正式显示使用一致的正文尺寸；长段落链接遵循按下、释放的完整点击判定。
