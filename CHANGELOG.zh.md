...
- **纸片缩放性能**：默认纸片和普通胶囊的阴影不再使用整块 WPF 模糊。拖动改变纸片大小时仍保留柔和外阴影，同时显著降低 CPU 占用和缩放卡顿。
- **皮肤实验**：保留默认纸片、云母、两种亚克力、描图纸、Aero 和像素风七种皮肤，均支持独立配色；已退役的皮肤 ID 统一回退默认纸片。Aero 保留清透染色、有限范围反光与随位置变化的视差，不读取桌面背景。
- **辅助材质改为静态背景快照**：云母、两种亚克力和描图纸的分层胶囊／菜单只在显示时读取一次有限范围的本地背景，并在显示期间冻结。拖动胶囊时只读取一次 50% 分辨率的整个虚拟桌面并做一次轻微高斯模糊，随后只移动裁剪；贴边胶囊和浮动拖动窗口共用同一张纹理。松手后再读取一次最终位置的局部背景。不再按 100/250ms 持续轮询桌面。
- **材质透明度五档**：新增最低 / 较低 / 中 / 较高 / 最高五档，其中“中”与修改前效果完全一致。该设置只调整原生与辅助材质的染色和覆盖强度，不使用 Window 整体透明度，因此文字和控件不会一起变淡；Aero 反光、立体边缘和描图纸纤维不随透明度档位消失。
- **原生背景**：云母、两种亚克力、描图纸和 Aero 在展开纸片与设置窗口使用原生背景，可保持材质的激活外观。需要 Windows 11 22H2 及以上，首次启用需退出并重新打开；Windows 11 24H2（26100）及以上通过独立透明通道让材质连续延伸到顶栏，不支持时保留原生兼容接法。部分透明以及系统不支持或关闭透明效果时保留实色底，高对比度隐藏装饰。换肤不重建编辑器和撤销记录。

- **插件读写与失败处理精简**：查询纸片、待办、笔记和图片不再提交其他正文，外部写入也不再全局 Commit 无关插件正文；如果写入目标就是同一张内置 Markdown，会先应用这张纸片尚未写回模型的用户文字，核心保存只同步内置 Markdown。插件状态只使用普通数据文件，读取失败直接报告，不再自动从空状态创建恢复文件；旧恢复文件不会被自动删除。普通主题、字号或激活通知出错不再撤掉整个插件正文；后台首次启动失败不再自动重试。后台已经成功运行后，Web renderer 最多原地重载一次，整个 Runtime 最多再重建一次；继续失败就停止，不再进入多级退避。插件数据连续保存失败时，也只额外延迟重试一次，不再持续快速后台重试。
- **插件交互与待办创建**：新建或追加待办时，初始完成状态、提醒和关联不再额外要求修改已有内容的权限。按钮和顶部标签的提示支持换行，小弹窗里的网页/邮件链接交给系统默认程序打开。复盘记录池不再依赖独立备份文件完成保存。
- **待办底栏与多选拖动**：待办纸片新增默认开启的底栏开关；关闭后同时隐藏底部快速新建区域并禁用拖入删除。多选待办现在可以作为一组直接拖动排序，并保持组内原有顺序。
...
- **启动显示器恢复精简**：坐标本身有效、但启动瞬间暂时不属于任何已报告显示器的纸片，会获得一次很短的一次性显示器宽限后再执行正常离屏救援；其他纸片仍立即恢复，不再使用轮询或多阶段拓扑等待。
