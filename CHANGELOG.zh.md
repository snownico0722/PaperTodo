...
- **边缘实时预览**：鼠标悬停在边缘胶囊即可浏览和操作内容，无需打开完整纸片即可即时浏览内容。
  ...
  - **待办实时预览**实时显示简化待办列表，支持直接滚动浏览、勾选完成或撤销，点击卡片背景可直接呼出完整纸片；关联或快启操作使用更稳定的独立点击区域
  - **笔记实时预览**：支持6000 字符，且同步笔记MD完整渲染。
...
- **便签微应用容器**：笔记纸可按需切换为独立的桌面微应用（如专注番茄钟、拟物时钟、复盘池或系统监控面板）；设置中新增独立「插件」管理中心，数据安全隔离保存在 plugins/data/ 目录下（10 MB / 20 MB 限制）。
  ...
  - 示例：
    - 示例插件：原生时钟、专注番茄钟、复盘记录池、Web 时钟等，适配窄窗口、胶囊与 Mini 视图。
    - 仓库 `plugin-samples/` 中含有完整示例源码
    - 仓库 `plugins/` 中的插件可直接使用。

#### 待办与 Markdown

- **全文搜索**：`Ctrl+F` 查找全部待办与笔记，显示本纸片 / 全局计数。`Enter` / `Shift+Enter` 或箭头循环定位命中项。
- **Markdown 所见即所得编辑**：新增完全渲染，达到真正MD完全渲染体验，支持块渲染，编辑时也可以完全渲染，不会跳变，体验基本超越Typora。
...
- **新增全局快捷键**：
  ...
  - 全部胶囊或当前纸片透明度
  - 将纸片 / 胶囊置底
- **失焦**：可让纸片失焦后自半透明/隐藏标题图标/隐藏标题栏/收起为胶囊
- **失焦透明度**或让普通/贴边胶囊在静置时自动半透明，并分别调整透明度。
- 贴边 / 主胶囊可取消强制置顶
...
- 优化胶囊展开后的焦点稳定性：从胶囊打开纸片后会更可靠地取得前台焦点，优化纸片已经展开但焦点错误。

- 修复快捷键页“恢复本页默认”时未恢复“小键盘数字键区分
