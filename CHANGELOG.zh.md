...
- **插件 API 2.2**：插件协议升级到 2.2，同时继续兼容 2.1 插件；`hidden` 启动纸片、设置页 `action` 按钮以及公共软件 Settings API/权限明确归入 2.2 合同。

- **软件设置 API**：MCP 与 Native/Web 插件可通过同一类型化目录查询、修改面向用户的设置，支持敏感设置独立授权和保存失败回滚；MCP 同时补齐待办解绑。
