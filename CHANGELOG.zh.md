...
- **皮肤实验**：保留默认纸片、云母、两种亚克力、描图纸、液态玻璃、Aero 和像素风八种皮肤，均支持独立配色；移除旧釉面皮肤及其绘制与资源，旧选择回退默认纸片。液态玻璃具有平滑曲面边缘、随尺寸适配的方向高光和真实背景 RGB 色散；中心背景保持原比例，调整大小不牵动正文区域的背景；小胶囊不再过度挤压边缘背景，光斑不随长宽比拉伸，指针离开后恢复环境高光，表面反光始终位于采样背景之上。Aero 保留清透染色和柔和斜反光，不增加磨砂或桌面采样。
- **实时背景处理**：纸片、设置、胶囊和菜单共享玻璃效果。胶囊与菜单可选完整材质；关闭时效果更轻、纸片底色更实，更便于阅读。边缘浏览使用固定色调与透明度的轻量材质；配色按不同材质分别调校。主胶囊、纸片和子菜单打开前准备当前材质及首张纹理，不叠加系统淡入；设置切页保留已有背景；皮肤设置精简为选项和提示图标。背景低频采样较大范围，小幅移动复用已有场景，拖动与光效按显示帧呈现，减少重复传输与绘制；背景换区时像素和坐标一起更新，采样不使用会扰动指针的旧式捕获标志。采样仅在本机内存处理，不保存或上传；可能影响截图、录屏和共享，分享前请关闭。云母／亚克力辅助表面为局部背景柔化近似；Aero 不采样。
- **原生背景**：云母、两种亚克力、描图纸、Aero 和仿液态玻璃在展开纸片与设置窗口使用原生背景，可保持材质的激活外观。需要 Windows 11 22H2 及以上，首次启用需退出并重新打开；Windows 11 24H2（26100）及以上通过独立透明通道让材质连续延伸到顶栏，不支持时保留原生兼容接法。部分透明以及系统不支持或关闭透明效果时保留实色底，高对比度隐藏装饰。换肤不重建编辑器和撤销记录。
...
- **Codex CLI Bridge**：默认模型更新为 GPT-6 Astra（`gpt-6-astra`）；推理档位仍为 `xhigh`，模型设置留空时仍不覆盖 Codex CLI 自己的模型配置。插件设置改用独立的**更多设置**页：启动、后台执行和模型保留在插件卡片上，推理档位与路径选项继续保留在完整设置页中。新增默认开启且互相独立的“开启插件技能”“开启本软件操作技能”“允许本插件开启 MCP”；技能改为发送时注入，默认提示词保持独立。任务需要 MCP 时，插件可自动开启 MCP 总开关及全部写入、删除权限，但不会改动普通 PaperTodo 功能开关。默认提示词还会要求待办保持简短，长内容仅在“待办关联纸片”已开启时拆到绑定 Note；笔记避免过度冗长，但也不做无意义的过度精简。Codex 提示词编辑纸片改为启动时默认隐藏，并在插件“更多设置”提供“编辑提示词”按钮直接展开打开。 MCP 按需开启命令会通过退出码返回宿主的实际执行结果，不再把“命令已发送”当作“已经开启”。
