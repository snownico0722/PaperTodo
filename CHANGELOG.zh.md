...
- **Codex CLI Bridge**：默认模型更新为 GPT-6 Astra（`gpt-6-astra`）；推理档位仍为 `xhigh`，模型设置留空时仍不覆盖 Codex CLI 自己的模型配置。插件设置改用独立的**更多设置**页：启动、后台执行和模型保留在插件卡片上，推理档位与路径选项继续保留在完整设置页中。新增默认开启且互相独立的“开启插件技能”“开启本软件操作技能”“允许本插件开启 MCP”；技能改为发送时注入，默认提示词保持独立。任务需要 MCP 时，插件可自动开启 MCP 总开关及全部写入、删除权限，但不会改动普通 PaperTodo 功能开关。默认提示词还会要求待办保持简短，长内容仅在“待办关联纸片”已开启时拆到绑定 Note；笔记避免过度冗长，但也不做无意义的过度精简。
