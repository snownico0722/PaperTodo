...
- 待办预览：
  ...
  - 实时显示简化待办列表，支持直接勾选完成或撤销，点击卡片背景可直接呼出完整纸片。
  - 关联或快启操作使用更稳定的独立点击区域，并按实际可用文字宽度排版，减少短文本被右侧操作挤换行。
  - 卡片只展示当前空间内的顶部内容，不支持滚动，溢出时在底部显示省略号。

- 笔记预览：

  - 跟随 Markdown 渲染设置，关闭时显示原文，基础采用原增强效果：保留标记、淡化语法并显示列表圆点和分割线，编号与任务状态保持清晰；完全渲染进一步隐藏语法标记；支持标题、强调、删除线、代码块、列表与图片占位。
  - 文字沿用正文的字体、字号、加粗、文字渲染设置及单篇缩放，普通行和空行不额外加宽间距。
  - 卡片只展示当前空间内的顶部内容，不支持滚动，溢出时在底部显示省略号；长笔记最多准备前 16 个内容块、6000 字符，卡片尺寸与显示使用同一份预览内容，不为预算外内容预留空白。
  - 行内语法沿用正文的识别实现，改善嵌套强调、转义与复杂链接的一致性。

<details>
<summary>笔记预览：排版、预热与复用细节</summary>

- 普通短行、长段落和代码块统一按可见空间在后台排版，减少对界面响应的占用及不必要的文字对象分配。

- 首次打开时正文会在第一批内容准备完成后一次显示，避免半成品内容闪现；相同内容和尺寸的预览在短暂收起后恢复时复用已完成结果，内容、字体、缩放或屏幕变化后重新准备。

- 启动恢复和内容变化后，会提前排好边缘笔记预览，反复切换时复用结果、减少首次浏览等待，不额外保留隐藏卡片。

- 开启边缘浏览且合格笔记不超过 10 张时，短笔记也全部预热，更多笔记时优先准备较长或样式较多的内容。

- 启动首批不额外等待编辑合并延迟，连续编辑仍合并处理，离开边缘队列后释放对应缓存。

- 预热与正式显示使用一致的正文尺寸；长段落链接遵循按下、释放的完整点击判定。

</details>

- 流畅接续与意图预测：

  - 内置可调节的鼠标移动意图预测，在胶囊间连续浏览时减少误切换。
  - 展开、收回时卡片与相邻胶囊同时运动，减少动画启动时的错位；优化连续浏览时的动画更新节奏，改善展开、收回和切换的更新连续性；单个队列更新不再暂停其他队列。
  - 默认向下切换时，只要目标卡片仍能完整显示，就优先保持当前鼠标附近的展开位置，不会为了填补上方空位突然跳动。
  - 常规设置可关闭「浏览时优先向下展开」，恢复优先利用上方空位的原补位规则。
  - 离开可浏览区域后自动收回；隐藏或恢复边缘队列时，正确结束旧的悬停状态。
  - 动画期间的鼠标按下保留原始位置和按键状态，仅在当次交接立即成功时转交，交接需要重试时不会迟到重放旧点击。
...
- 胶囊尺寸与计数优化：
  ...
  - 待办与笔记边缘胶囊统一图标槽和标题起点，隐藏关闭按钮时不再保留空白占位。
  - 主胶囊展开或收起时都始终显示准确数量，1～99 保持紧凑的两位数固定宽度，100+ 按实际数量扩展，状态切换时不再因数字和箭头变化左右伸缩。
...
- **Codex CLI Bridge**：
  ...
  - 待办项与纸片顶栏可直接把内容交给本机 Codex CLI，支持图片附件、绑定路径及关联纸片上下文。
  - 专属纸片可编辑默认提示词，未编辑时使用内置提示词，已编辑或主动清空的内容会保留。
  - 随插件提供 PaperTodo 插件制作 Skill 和开发手册，默认在提出制作插件的需求时调用。
...
- 示例和提醒：
  ...
  - 示例：仓库 `plugin-samples/` 随附原生时钟、专注番茄钟、复盘记录池、Web 时钟等完整示例源码，适配窄窗口、胶囊与 Mini 视图。仓库 `plugins/` 中的插件可直接使用。

<details>
<summary>Web / Native 双模运行时架构</summary>
...
- **内置全文搜索**：
  ...
  - `Ctrl+F` 可跨全部待办和 Markdown 笔记查找，包含已完成的待办，显示当前纸片与全局命中计数。
  - `Enter` / `Shift+Enter` 或箭头按钮可循环跳转并定位到具体待办项或笔记文字，自动显示隐藏纸片、展开胶囊并接续搜索。
  - 搜索框保持输入焦点时，当前待办或笔记命中仍会保持可见高亮。
  - 搜索框唤出后可直接输入，从纸片内展开并允许超出边框；可通过靠右的圆角竖条拖动柄、计数区或空白区域移动，采用无阴影样式，上下箭头和关闭按钮使用矢量图标。

- **完全渲染升级为所见即所得块级编辑**：

  - Markdown 显示简化为「关闭 / 基础 / 完全渲染」；旧基础与旧增强统一为新的基础，其中基础采用旧增强效果；完全渲染保持不变，旧配置自动迁移，新安装及恢复默认选择基础。
  - 笔记选择「完全渲染」档，编辑时也按标题、列表、引用、代码块、图片与行内样式等最终排版直接显示，并隐藏多数 Markdown 标记。
  - 标题、加粗、链接等文字紧凑重排；任务框、无序列表和引用使用稳定占位，减少勾选、编辑切换与引用续行时的横向跳动。
  - 完全渲染中的任务复选框可直接点击勾选或取消，实际写回原始 Markdown `[ ]` / `[x]` 并正常进入撤销 / 重做历史。
  - 有序列表编号始终直接显示原文字，避免编辑切换时闪动。
  - 光标所在块会显示其源码标记，失焦回到整篇只读渲染。
...
- 复制格式转换：
  ...
  - 待办多选可用 `Ctrl+Shift+C` 或右键“复制为 Markdown”生成保留完成状态的 Markdown 任务列表。
  - 笔记正文可用同一快捷键或右键“复制为纯文本”去除 Markdown 标记后复制。
  - 普通 `Ctrl+C` 保持原有复制方式。
...
- 修复开启「全屏时避让置顶窗口」后，部分以**管理员权限运行的全屏程序**前台时，PaperTodo 纸片仍可能显示在全屏画面上方的问题。
...
- **启动与退出响应**：
  ...
  - 显示器尚未就绪时，只延后相关纸片，其他胶囊和纸片可先使用。
  - 边缘笔记先准备可浏览的预览，再按低优先级短批次补建折叠纸片，右键菜单和可选初始化不阻挡首轮恢复。
  - 插件启动纸片不再轮询等待。
  - 正常退出保留最后一次保存，随后先撤下界面，同时停止脚本进程，并完成正常窗口退出流程，减少逐个等待和进程结束拖延。
...
- **自定义纸片背景**：
  ...
  - 在 `PaperTodo.exe` 同目录放入 `papertodo.png`、`papertodo.jpg` 或 `papertodo.jpeg`，即可为待办与内置 Markdown 笔记共用自定义背景。
  - 「视觉」设置可选择是否与当前纸片配色混合，关闭混合时仍显示原图。
  - 支持拉伸、居中、左下、正下、右下；除拉伸外保持原始比例完整显示、不裁切图片。
  - 图片损坏或无法加载时安全回退并在设置中显示提示；超大图片按最长边 4096 像素限制解码，小图保持原始尺寸。

- 设置面板重排：

  - 设置窗口改为左侧导航，按常规、待办、笔记、外观、快捷键、插件、实验室分类。
  - 窗口与胶囊选项并入常规，常规 / 待办 / 笔记可分别恢复本页默认，每页独立记忆滚动位置，高级模式固定在侧栏底部；「帮助改进」移入常规并简化说明。
  - 小屏幕、高缩放和跨不同 DPI 显示器时会按当前工作区重新适配。
...
- 失焦形态与置顶自定义：
  ...
  - 支持纸片失焦时仅隐藏顶栏按钮，或淡出标题栏并保留完整的圆角、描边和阴影，完全隐藏后可点击到后面的窗口，正文位置和窗口尺寸保持不变。
  - 支持让贴边胶囊与主胶囊不再强制保持系统最前台置顶。
