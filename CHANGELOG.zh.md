...
- **皮肤实验**：保留默认纸片、云母、两种亚克力、描图纸、Aero 和像素风七种皮肤，均支持独立配色；已退役的皮肤 ID 统一回退默认纸片。Aero 保留清透染色、有限范围反光与随位置变化的视差，不增加磨砂或桌面采样。
- **辅助背景处理**：云母、两种亚克力和描图纸的分层胶囊／菜单可处理有限范围的本地背景，用于材质柔化；完整档和弱档共用相同正文与命中逻辑。采样仅在本机内存处理，不保存或上传；开启处理的窗口可能被截图、录屏或共享排除，分享前请关闭。展开纸片与设置窗口直接使用原生背景，不走软件采样；Aero 不采样。
