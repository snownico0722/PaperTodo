...
- **Codex CLI Bridge**：默认模型更新为 GPT-6 Astra（`gpt-6-astra`）；推理档位仍为 `xhigh`，模型设置留空时仍不覆盖 Codex CLI 自己的模型配置。插件设置改用独立的**更多设置**页：启动、后台执行和模型保留在插件卡片上，推理档位与路径选项继续保留在完整设置页中。新增默认开启且互相独立的“开启插件技能”“开启本软件操作技能”“允许本插件开启 MCP”；技能改为发送时注入，默认提示词保持独立，MCP 可在写入任务需要时按需开启。默认提示词还会要求待办保持简短，长内容可拆到绑定 Note；笔记避免过度冗长但不做无意义压缩。
