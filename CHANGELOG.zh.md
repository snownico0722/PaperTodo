...
- 笔记预览跟随 Markdown 档位：关闭显示原文；基础沿用旧增强效果，保留淡化标记、列表圆点和分割线，编号与任务状态保持清晰；完全渲染隐藏标记。支持标题、强调、删除线、代码、列表和图片占位，沿用正文的字体、字号、加粗、文字渲染及单篇缩放，不额外增加行距。
- 预览只显示顶部内容，不滚动，溢出显示省略号；笔记最多 16 块、6000 字符，尺寸按同一份内容计算，不留多余空白。行内语法与正文一致，支持嵌套强调、转义和复杂链接；长段落链接需完整按下、释放。
- 预览按可见空间分批在专用后台线程排版，减少界面占用和临时对象；首批正文准备好后整体显示。短暂收回可复用未变的内容与尺寸，内容、字体、缩放或屏幕变化后重排。
- 启动及编辑后提前准备预览，切换时复用，不保留隐藏卡片；开启边缘浏览且合格笔记 ≤10 张时全部预热，否则优先长文和复杂样式。启动首批不等编辑延迟，连续编辑合并处理；预热与显示尺寸一致，离队释放缓存。
- 连续浏览支持可调鼠标意图预测，减少误切换；卡片与相邻胶囊同步展开 / 收回，动画更连贯，各队列独立更新。向下浏览默认就近展开，能完整显示时不为补上方空位而跳动；常规设置可关闭「浏览时优先向下展开」恢复旧规则。
- 离开浏览区自动收回，隐藏 / 恢复队列清除旧悬停状态；拖动排序或移出时收起预览，空纸片使用紧凑尺寸。动画点击保留按下位置和按键状态，仅交接立即成功时转交，重试不重放旧点击。
- 重复点击胶囊：纸片明显可见时收回，被明显遮挡时优先带回前台。标题支持隐藏（0）或不限长度，加减循环；关闭预览时，截短标题随悬停展开、离开收回，不改原标题。
- 胶囊图标与标题对齐，隐藏关闭按钮不留空位；主胶囊展开 / 收起均显示准确计数，1–99 固定两位宽，100+ 按需扩宽，不再因数字和箭头变化抖动。

#### 插件系统与桌面微应用

- **桌面微应用**：笔记纸可切换为番茄钟、时钟、复盘池或监控面板；设置新增「插件」中心，数据独立保存在 `plugins/data/`。
- **Codex CLI Bridge**：从待办或顶栏发送内容、图片、绑定路径及关联纸片上下文到本机 Codex CLI。专属纸片可改默认提示词，未改用内置，修改及主动清空均保留；附插件制作 Skill 和手册，制作插件时默认调用。
- Web 插件使用 WebView2 + HTML/CSS/JS，无需编译；Native 插件基于 .NET 10 + WPF，支持高性能自绘。运行部分分为正文、Mini 悬停和常驻后台。
- 插件可自定义胶囊（图标、文字、动态进度条 / 环、自绘）及 Mini 预览，提供待办操作图标、待办与纸片右键文字入口、顶栏按钮 / 状态、全局快捷键，并独占接管 `Esc` 与正文菜单；可读取获准的笔记图片，打开就近且失焦即关的弹窗。
...
- **全文搜索**：`Ctrl+F` 查找全部待办（含已完成）与内置 Markdown 笔记，显示本纸片 / 全局计数。`Enter` / `Shift+Enter` 或箭头循环定位命中项，自动显示隐藏纸片、展开胶囊并跨纸片接续；输入时保留命中高亮。
- 搜索框即开即输，可超出纸片边框，通过右侧圆角拖柄、计数或空白区移动；无阴影，箭头和关闭按钮使用矢量图标。
- **Markdown 所见即所得编辑**：完全渲染时边写边看标题、列表、引用、代码块、图片及行内样式，隐藏多数标记；光标所在块显示源码，失焦回到只读渲染。标题、强调和链接紧凑重排；任务框、无序列表和引用保持占位，有序编号保留原文，减少编辑、勾选和续行时跳动。
- 渲染档位简化为「关闭 / 基础 / 完全渲染」：旧基础、旧增强自动归入新基础，采用旧增强效果；完全渲染不变，新安装及恢复默认选择基础。渲染后的任务框可直接勾选，写回 `[ ]` / `[x]` 并支持撤销 / 重做；完全渲染可选短淡入动画，其他档位不显示该设置。
- **待办多选与批量操作**：沿左侧滑动连续选中多条，批量勾选 / 恢复、复制、右键删除或整组拖入垃圾箱。方向键可跨项连续编辑，长按不连跳；纸片上限从 100 提至 200 张，达到上限仍有清理提示。
- `Ctrl+Shift+C` 可将多选待办复制为保留完成状态的 Markdown 任务列表，或将笔记选区复制为纯文本；右键有对应入口，普通 `Ctrl+C` 不变。
...
- **全屏避让**：修复开启「全屏时避让置顶窗口」后，纸片仍可能遮挡部分管理员权限全屏程序的问题。
...
- **自定义纸片背景**：在 `PaperTodo.exe` 旁放入 `papertodo.png`、`papertodo.jpg` 或 `papertodo.jpeg`，待办与内置笔记共用。视觉设置可选原图或叠加纸片配色，支持拉伸 / 居中 / 左下 / 正下 / 右下；非拉伸保持比例、不裁切。加载失败会回退并提示，解码最长边限 4096 像素，小图保留原尺寸。
- 设置改为左侧导航，分常规、待办、笔记、外观、快捷键、插件、实验室；窗口 / 胶囊选项并入常规，常规 / 待办 / 笔记可恢复本页默认，各页记忆滚动位置，高级模式固定底部，“帮助改进”移入常规并简化说明。
- 设置适配小屏、高缩放与跨 DPI 切屏；开关和快捷键录制局部刷新，减少闪动。
- 新增全局快捷键：锁定全部纸片，切换全部纸片、全部胶囊或当前纸片透明度；当前纸片支持 `Ctrl+W`，标题栏中键与右上角按钮执行相同的关闭 / 收起动作。
- 支持失焦自动收起，编辑、拖拽、菜单及 Passive 交互不误触发；高级模式可保留关联纸片在快捷隐藏前的隐藏状态，避免恢复时意外弹出。

#### 实验室功能

- **本地 MCP**：以 `--mcp` 启动标准服务，外部 AI 可按授权读取、新建、追加和管理笔记 / 待办；设置可复制配置和 AI Skill 提示词。
- 窗口绑定：将顶栏按钮拖到第三方窗口，纸片贴靠并平滑跟随移动、最小化和恢复。浮动胶囊拖近屏幕或外部窗口边缘可磁吸，悬停滑出、离开收回。
...
- 修复快捷键页恢复默认时未恢复“小键盘数字键区分”，以及设置下拉菜单未完整跟随主题配色的问题。
